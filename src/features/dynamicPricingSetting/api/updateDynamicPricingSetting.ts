...
export const useUpdateDynamicPricingSetting = ({ config }: UseUpdateDynamicPricingSettingOptions = {}) => {
  const { addNotification } = useNotificationStore();

  return useMutation({
    onMutate: async (updatingDynamicPricingSetting: any) => {
      await queryClient.cancelQueries(['dynamicPricingSetting', updatingDynamicPricingSetting?.dynamicPricingSettingId]);

      const previousDynamicPricingSetting = queryClient.getQueryData<IDynamicPricingSetting>([
        'dynamicPricingSetting',
        updatingDynamicPricingSetting?.dynamicPricingSettingId,
      ]);

      queryClient.setQueryData(['dynamicPricingSetting', updatingDynamicPricingSetting?.dynamicPricingSettingId], {
