...
export const createTimeRule = async (timeRuleDto: CreateTimeRuleDTO): Promise<ITimeBasedTriggerRule> => {
  const { data } = await axiosInstance.post("/api/v1/rms/time-based-trigger-rule/", snakify(timeRuleDto.data)) as { data: ITimeBasedTriggerRule };
...
export const useCreateTimeRule = ({ config }: useCreateTimeRuleOptions = {}) => {
